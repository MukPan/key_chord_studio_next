...
//全12個
  ...
  let nextChordNameList = []; //予測コードリスト
    ...
    for (let i = (calcArray.length); i > 0; i--) { //右から順番に-12する
      ...
      if (RootsArr[originRoot] !== undefined && i === calcArray.length) { //structは完全一致しないけど、一部一致したものを探そう(転回系は考えない)
        if (selectBoxValue.value === "simiR") { //類似(ルート音固定)
          nextChordNameList = Object.keys(Chords)
          ...
          nextChordNameList = Object.keys(allParaDistsObj)
  ...
  }, [isSelectedArr]); //Selectで使用するuseState変数はここにぶち込んでおく //, selectBoxValue, sortType
...
onContextMenu={() => addToDisplay(chord)}
  ...
  return (
    <>
      ...
      {/*次コード*/}
      <div style={predictChordAreaStyle}>
        <p style={headlineTextStyle}>次候補<br/>コード</p>
        <div style={predictChordDisplayStyle}>
          <p id="main-display">
            {hitElemArr.map(e => e)}
          </p>
